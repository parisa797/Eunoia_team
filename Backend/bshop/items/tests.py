...
class TestRateItemCreate(APITestCase):
    ...
    def test_create_rate(self):
        ...
        response = self.client.post(
            reverse('RateCreate', kwargs={
                    'pk': self.sh7.pk, 'id': self.I4.id}),
...
class TestRateItemList(APITestCase):
    ...
    def test_list_all_rate(self):
        ...
        response = self.client.get(
            reverse('RateCreate', kwargs={'pk': self.sh5.pk, 'id': self.I4.id}))
...
class TestRateRetrieveUpdateDestroyItem(APITestCase):
    ...
    def test_delete_rate(self):
        response = self.client.delete(reverse('RateRetrieveUpdateDestroy', kwargs={
                                      'pk': self.sh.pk, 'id': self.I4.id, 'rate_id': self.r4.id}))
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)

    def test_update_rate(self):
        response = self.client.put(
            reverse('RateRetrieveUpdateDestroy', kwargs={
                    'pk': self.sh.pk, 'id': self.I4.id, 'rate_id': self.r4.id}),
            data=json.dumps(self.valid_rate),
            content_type='application/json'
        )
        ...
        response = self.client.get(reverse('RateRetrieveUpdateDestroy', kwargs={
                                   'pk': self.sh.pk, 'id': self.I4.id, 'rate_id': self.r5.id}))
...
class TestSpecialItem(APITestCase):
    ...
    def setUp(self):
        ...
        self.user2 = MyUser.objects.create(
            username="test2", email="test1@gmail.com", password="strong_Pass")
        self.token = Token.objects.create(user=self.user)
        self.token2 = Token.objects.create(user=self.user2)
        self.client.credentials(HTTP_AUTHORIZATION="Token " + self.token.key)

    def test_create_special_item(self):
        response = self.client.post(reverse('specialitem_create'), {
...
class TestSpecialItemRetreive(APITestCase):
    ...
    def test_get_Special_item(self):
        response = self.client.get(
            reverse('specialitem_RUD', kwargs={'pk': self.specialItem.pk}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data, SepcialItemSerializer(self.specialItem).data)
    ...
    def test_coin_shop_retrieve(self):
        self.client.credentials(HTTP_AUTHORIZATION="Token " + self.token2.key)
        response = self.client.get(
            reverse('specialitem_retrieve', kwargs={'pk': self.specialItem.pk}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
