...
class Media(models.Model):
    uploaded_file = models.ImageField(
        upload_to='profiles/', max_length=100, null=True, blank=True)
...
class MyUser(AbstractUser):  # ?
    ...
    role = models.CharField(
        max_length=50, choices=role_choices, default='buyer')
...
class Coins(models.Model):
    ...
    rank = models.CharField(
        max_length=50, choices=rank_choices, default='none')
    money = models.IntegerField(default=0, blank=True, null=True)
    user = models.ForeignKey(MyUser, blank=True, null=True,
                             on_delete=models.CASCADE, related_name='usercoins')
    last_buy = models.DateTimeField(auto_now_add=False, blank=True, null=True)


class ShoppingWithCoin(models.Model):
    coin = models.ForeignKey(Coins, blank=True, null=True,
                             on_delete=models.CASCADE, related_name='coinshop')
    special_item = models.ForeignKey(
        "items.SpecialItem", blank=True, null=True, on_delete=models.CASCADE, related_name='Specialitemforbuy')


class ElectricWallet(models.Model):
    money = models.IntegerField(default=0, blank=True, null=True)
    user = models.ForeignKey(MyUser, blank=True, null=True,
                             on_delete=models.CASCADE, related_name='userwalllet')
