...
import json
from .models import MyUser,Coins
from .serializers import Profileserializer,CoinSerializer,ShoppingWithCoinSerializer
...
class CoinRetriveTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.sl = ShoppingList.objects.create(shop=self.sh, user=self.user, address="address 1", phone="111111",date="2021-04-12 12:30")
        self.il=ShoppingItem.objects.create(item=self.I,number=4,shopping_list=self.sl,price=200000,sum_price=800000)
    ...
    def test_get_coin(self):
        response = self.client.get(reverse('coinInfo',kwargs={'pk': self.coin.pk}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data, CoinSerializer(self.coin).data)


    def test_update_coin(self):
        response = self.client.put(reverse('coinInfo',kwargs={'pk': self.coin.pk}),{'money':500})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        coin=Coins.objects.get(user=self.user)
        self.assertEqual(response.data, CoinSerializer(coin).data)
        self.assertEqual(response.data['money'], 500)

        response=self.client.put(reverse('coin_update',kwargs={'pk': self.coin.pk}))
    ...
    def test_delete_coin(self):
        response = self.client.delete(reverse('coinInfo', kwargs={'pk': self.coin.pk}))
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)

    def test_sabt_update_coin(self):
        response = self.client.put(reverse('sabt_shopping_list',kwargs={'pk': self.sl.pk}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        coin=Coins.objects.get(user=self.user)
        self.assertEqual(coin.money, 180)

    def test_coin_shop(self):
        response = self.client.post(reverse('coin_shop',kwargs={'pk': self.specialItem.pk}))
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data, "You got this award. Please visit this address between 9 am and 4 pm with the national card to receive the award")
        coin=Coins.objects.get(user=self.user)
        self.assertEqual(coin.money, 50)

        response = self.client.post(reverse('coin_shop', kwargs={'pk': self.specialItem2.pk}))
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_coin_shop_list(self):
        self.client.post(reverse('coin_shop', kwargs={'pk': self.specialItem.pk}))
    ...
    def test_coin_shop_retrieve(self):
        self.client.post(reverse('coin_shop', kwargs={'pk': self.specialItem.pk}))
        response = self.client.get(reverse('retrieve_coin_shop', kwargs={'pk': self.sl.pk}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
